...
class LFUCache(BaseCaching):
    """ LFUCache is a caching system with an LFU eviction policy
    """
    ...
    def put(self, key, item):
        ...
        if key in self.cache_data:
            ...
            if len(self.cache_data) >= BaseCaching.MAX_ITEMS:
                ...
                lfu_keys = [k for k, freq in self.usage_frequency.items() if freq == min_freq]
                if len(lfu_keys) == 1:
                    lfu_key = lfu_keys[0]
                else:
                    # If there's a tie, use the order to find the least recently used
